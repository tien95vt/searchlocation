	...
	<section>
		...
		<div class="container">
			...
			<ul class="nav nav-tabs" id="myTabs">
				...
				<li><a href="#profile" data-url="/embed/62801/view">Nhà hàng</a></li>
				<li><a href="#cafe" data-url="/embed/62806/view">Café/Dessert</a></li>
		...
		{{-- Thể loại khác, còn lại --}}
		<div class="tab-pane" id="messages">
			<div class="panel-body content_ajax_page_khac">
				@php $breakRow = 1; @endphp
				<div class="row" style="margin: 5px;">
...
<script>
	...
	// Ajax page khac
	$(document).ready(function() {
		$(document).on('click', '.pageKhac .pagination a',function(e){
			e.preventDefault();	//Dừng refresh page
			var pageKhac = $(this).attr('href').split('pagination_khac=')[1];
			getProductKhac(pageKhac);
		});
	});

	function getProductKhac(pageKhac){
		$.ajax({
			url: 'ajax/product_pageKhac?pagination_khac='+ pageKhac //?pagination_khac => Quan trọng
		}).done(function(data){
			// console.log(data);
			$(".content_ajax_page_khac").html(data);
		});
	}
	// End Ajax page khac

	// // // Ajax pagination cafe
	$(document).ready(function() {
		$(document).on('click', '.page_cafe .pagination a', function(e){
			e.preventDefault();
			var pageCafe = $(this).attr('href').split('pagination_cafe=')[1];
			getProductPageCafe(pageCafe);
		});
	});
	function getProductPageCafe(pageCafe){
		$.ajax({
			url: 'ajax/product_pageCafe?pagination_cafe='+ pageCafe	//?pagination_cafe => Quan trọng
		}).done(function(data){
			// console.log(data);
			$(".content_ajax_pageCafe").html(data);
		});
	}
	// End  Ajax pagination cafe
